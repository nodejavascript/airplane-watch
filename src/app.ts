 ...
 */
function runText(minutes: number): string {
...
class Page {
   ...
   */
  private tookOffAt = new Map<string, number>();
  private firstSeenAirborne = new Map<string, number>();
   ...
   */
  private airportCoords = new Map<string, { lat: number; lon: number } | null>();
  private askingAirport = new Set<string>();
  /** Airports waiting their turn — see `AIRPORT_LOOKUPS_AT_ONCE`. */
  private airportWaiting: string[] = [];
  /** When a REFUSED airport may be asked about again — see `AIRPORT_RETRY_MS`. */
  private airportRetryAt = new Map<string, number>();
  ...
  private renderAircraft(): void {
    ...
    for (const { state } of rowsSorted) {
      ...
      html.push(
        ...
        }${/* 🔴 THE TAIL SITS UNDER THE AIRCRAFT TYPE, AND IT NO LONGER DEPENDS ON THE TYPE TABLE. George,
        ...
        registration prints the tail under the type and not twice in the same row. */
        tailUnderType ? `<span class="cell-tail">${escapeHtml(tailReg)}</span>` : ''}</td>` +
   ...
   */
  private noteTimes(readings: Reading[], departures: Departure[]): void {
    const now = Date.now();
    ...
    for (const departure of departures) {
      ...
      if (hex === '' || departure.verdict !== 'confirmed') continue;
   ...
   */
  private async lookUpAirport(key: string): Promise<void> {
    try {
      ...
      const trouble = this.feedTrouble(response);
      if (trouble) throw new Error(trouble);
      const body = (await readJson(response)) as { lat?: number; lon?: number };
   ...
   */
  private runToDestination(
    state: TrackState,
    icao: string
  ): { minutes: number; nm: number; knots: number } | null {
    ...
    if (!airport || lat === null || lon === null || knots === null) return null;
    ...
    if (!Number.isFinite(minutes) || minutes < 1 || minutes > 12 * 60) return null;
   ...
   */
  private departureCell(route: RouteInfo | null | undefined, state: TrackState): string {
    ...
    const stamp = tookOff !== null ? clockTime(tookOff) : cameIntoView ? clockTime(firstSeen as number) : '';
    const timeWhy =
      tookOff !== null
    ...
    if (route === null) {
      return (
        `<td class="mono dest dest-none" title="${escapeHtml(
          'No route is on file for this callsign. The aircraft itself never transmits where it came from.' + noTime
        )}">` +
    ...
    const from = [stripBrackets(origin.city), origin.country]
      ...
      .join(', ');
    const whole =
      `On file for this callsign: it left ${origin.icao} ${origin.city}.` +
   ...
   */
  private destinationCell(route: RouteInfo | null | undefined, state: TrackState): string {
    if (route === undefined) {
      return (
        '<td class="mono dest dest-waiting" title="Asking the route lookup about this callsign.">' +
    ...
    const runLine =
      run === null
        ? ''
        : `<span class="leg-time" title="${escapeHtml(
            `About ${Math.round(run.nm)} nautical miles still to run at the ${Math.round(run.knots)} knots ` +
              'it is reporting over the ground. That assumes a straight line and the same speed all the way, ' +
              'so it is an estimate rather than an arrival time.'
          )}">in about ${escapeHtml(runText(run.minutes))}</span>`;
    const whole =
      `On file for this callsign: it is bound for ${destination.icao} ${destination.city}` +
      `${airline ? ` · ${airline}` : ''}.` +
      ' A route is looked up, not transmitted by the aircraft, so a diversion or a reused callsign can make it wrong.';
    return (
      `<td class="mono dest" title="${escapeHtml(whole)}">` +
  ...
  private bindFlightPick(): void {
    document.addEventListener('click', (event) => {
      ...
      if (target.closest('button, a, input, .tail-chip')) return;

      const row = target.closest<HTMLElement>('tr.aircraft-row');
      if (row) {
    ...
    document.addEventListener('keydown', (event) => {
      ...
      const target = event.target as HTMLElement | null;
      const row = target?.closest<HTMLElement>('tr.aircraft-row, li.watch-type[data-hex]');
      if (!row) return;
  ...
  private renderWatchlist(): void {
    ...
    const namedItems = this.watchlist
      .map((item) => {
        ...
        const flying = live.find(
          (one) =>
            normaliseKey(one.registration) === normaliseKey(item) &&
        ...
        return (
          `<li class="watch-type${picked ? ' row-selected' : ''}"` +
          (hex !== ''
