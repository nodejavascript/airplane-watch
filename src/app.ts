...
class Page {
  ...
  private tickReadingAges(): void {
    const paint = (): void => {
      ...
      if (refreshed) refreshed.textContent = this.lastPollAt === 0 ? 'not yet' : fromNow(this.lastPollAt);
    };
    paint();
            ...
            : '<div class="watch-tails">' +
              tails
                .map((tail) => {
                  ...
                  return (
                    `<span class="tail-chip" data-watched="${on}" ` +
                    `title="${escapeHtml(
                      on
                        ? narrowed
                          ? 'Watching this aeroplane'
                          : 'The whole type is watched, so this one is too'
                        : 'Not watched — tick it in step 3 to watch only this aeroplane'
