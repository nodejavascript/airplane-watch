...
(function () {
  ...
  function redact(value: unknown, limit: number): string {
    ...
    return value
      .replace(/[a-z][a-z0-9+.-]*:\/\/[^\s)"']*/gi, function (url: string) {
        return withoutQuery(url);
      })
      .replace(/(^|[\s("'=])(\/[^\s)"']*)/g, function (_whole: string, lead: string, path: string) {
  ...
  window.addEventListener(
    ...
    function (event: Event) {
      try {
        ...
        const failure = (event as ErrorEvent).error as Error | undefined;
        report(
          (event as ErrorEvent).message,
          failure && failure.stack ? failure.stack : '',
          (event as ErrorEvent).filename,
          (event as ErrorEvent).lineno,
