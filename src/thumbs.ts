...
import type { AircraftClass } from './typeinfo.js';

/** The drawings. One per shape of aeroplane, named for the shape. */
type Shape =
  | 'jet'
  | 'turboprop'
  | 'bizjet'
...
const SHAPES: Record<Shape, string> = {
  ...
  bizjet:
    '<path d="M24 6c2 0 3 2 3 4.6V37c0 2.8-1 4.6-3 4.6s-3-1.8-3-4.6V10.6C21 8 22 6 24 6Z"/>' +
  ...
  heavy:
    '<path d="M24 5c2.8 0 4.2 2.6 4.2 6v27c0 3.4-1.4 5.8-4.2 5.8S19.8 41.4 19.8 38V11c0-3.4 1.4-6 4.2-6Z"/>' +
  ...
  fighter:
    '<path d="M24 4.5c1.8 0 2.6 2 2.6 4.4V38c0 3-0.8 5-2.6 5s-2.6-2-2.6-5V8.9c0-2.4 0.8-4.4 2.6-4.4Z"/>' +
 ...
 */
export function shapeFor(code: string, klass: AircraftClass): Shape {
  const key = String(code ?? '').trim().toUpperCase();
  if (HEAVY.has(key)) return 'heavy';
  if (FIGHTER.has(key)) return 'fighter';
  if (TURBOPROP.has(key)) return 'turboprop';
  if (BIZJET.has(key)) return 'bizjet';
 ...
 */
export function thumbSvg(code: string, klass: AircraftClass): string {
  const shape = shapeFor(code, klass);
  return (
    `<svg viewBox="0 0 48 48" class="thumb-${shape}" width="40" height="40" aria-hidden="true" focusable="false">` +
