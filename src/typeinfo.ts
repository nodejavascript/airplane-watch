 ...
 */
const MAKER_PREFIXES: readonly (readonly [string, string])[] = [
 ...
 */
export function makerOf(code: string | undefined | null): string | null {
  const { name, known } = describeType(code);
  if (!known) return null;
  for (const [prefix, maker] of MAKERS_BY_LENGTH) {
    if (name.startsWith(prefix)) return maker;
  }
  return null;
}
