 ...
 */
import { parseAirport, } from './airports.js';
 ...
 */
function climbHue(from, to, current) {
    if (from !== null && to !== null) {
        const delta = to - from;
        if (delta >= TRAIL_LEVEL_FT)
            return 'climb';
        if (delta <= -TRAIL_LEVEL_FT)
            return 'descend';
        return 'level';
 ...
 */
const PLANE_PATH = 'M12 2 L13.6 2.4 L14.4 9 L21 12.4 L21 14.2 L14.4 13.2 L14.2 18 L16.6 19.4 L16.6 20.6 ' +
...
const BELL_PATH = 'M12 2.6a5.6 5.6 0 0 0-5.6 5.6v3.2l-1.5 2.6a1 1 0 0 0 .87 1.5h12.46a1 1 0 0 0 .87-1.5l-1.5-2.6V8.2A5.6 5.6 0 0 0 12 2.6Zm0 18.8a2.6 2.6 0 0 0 2.45-1.8h-4.9A2.6 2.6 0 0 0 12 21.4Z';
/**
 ...
 */
function bellButton(code, wholeType, narrowed) {
    ...
    return (`<button type="button" class="bell alert-toggle${narrowed ? ' bell-part' : ''}" ` +
 ...
 */
function storedKeys(includeConsent) {
    const found = [];
    try {
...
function forgetStored(includeConsent) {
...
class Page {
    ...
    async toggleAirport(icao) {
        ...
        if (listed) {
            ...
            try {
                ...
                if (!response.ok)
                    throw new Error(`The feed answered ${response.status} while resolving an airport.`);
     ...
     */
    async refreshFilterResults() {
        if (this.filterRefreshPending)
            return;
        this.filterRefreshPending = true;
        const button = byId('refreshFilters');
    ...
    async poll() {
        ...
        try {
            ...
            for (const reading of readings) {
                ...
                if (!this.insideMyCircle(reading.lat, reading.lon))
                    continue;
            ...
            const changed = seen.size !== this.liveTypes.size || [...seen.keys()].some((code) => !this.liveTypes.has(code));
            this.liveTypes = seen;
            if (changed)
                this.renderTypeList();
    ...
    watchStateOf(code, live) {
        ...
        const count = Page.countMatching(live, (one) => one.phase === 'airborne' && normaliseKey(one.type ?? '') === normaliseKey(code));
        if (count > 0) {
            return {
                kind: 'air',
                text: count === 1 ? 'in the air' : `${count} in the air`,
                why: 'The feed is hearing this type AIRBORNE inside your fence right now, so it is on the map ' +
    ...
    watchStateOfTail(tail, live) {
        ...
        const count = Page.countMatching(live, (one) => one.phase === 'airborne' && normaliseKey(one.registration ?? '') === normaliseKey(tail));
        if (count > 0) {
            return {
                kind: 'air',
                text: 'in the air',
    ...
    tickWatchStates() {
        ...
        const drawnAgeKey = [...this.mapLastDrawn]
            ...
            .sort()
            .join(',');
        const statusKey = `${liveKey}|${drawnAgeKey}|${this.surveyRead ? (this.survey?.generated ?? 'read') : 'unread'}` +
    ...
    withNeverCaught(recorded) {
        ...
        return [...recorded, ...extra].sort((a, b) => describeType(a.code).name.localeCompare(describeType(b.code).name) || a.code.localeCompare(b.code));
    }
    /**
     ...
     * than decorative.
     */
    droppedReasons(counts) {
        const parts = [];
        if (counts.droppedByAirport > 0) {
            parts.push(`${counts.droppedByAirport} ${counts.droppedByAirport === 1 ? 'type has' : 'types have'} been seen around ` +
        ...
        if (counts.droppedForNoYear > 0) {
            parts.push(`${counts.droppedForNoYear} ${counts.droppedForNoYear === 1 ? 'has' : 'have'} no first-flown year, ` +
                'so a year filter cannot judge them');
        }
        if (counts.droppedBySeen > 0) {
            parts.push(`${counts.droppedBySeen} ${counts.droppedBySeen === 1 ? 'is' : 'are'} outside the last-seen choice you made`);
        }
        return parts.join('; ');
    }
    /**
    ...
    runsRecorded() {
        ...
        return Number.isFinite(n) && (n ?? 0) > 0 ? n : Math.max(1, ...(this.survey?.types ?? []).map((row) => row.runsSeen ?? 1));
    }
    ...
    renderFilterNote() {
        ...
        if (rows.length > 0) {
            ...
            if (body)
                body.textContent = `${parts.join(' ')} ${words}`;
    ...
    renderAreaPicker() {
        ...
        host.innerHTML =
            `<span class="small muted">Which one? </span>` +
                this.placeAreas
                    .map((area) => `<button type="button" class="chip chip-small area-chip" data-area="${escapeHtml(area)}" ` +
     ...
     */
    canAskInPage() {
        const dialog = byId('removeDialog');
    ...
    bindRemoveAsk() {
        ...
        byId('removeGo')?.addEventListener('click', () => {
            ...
            if (pending.kind === 'type')
                this.removeTypeRule(pending.key);
            else
                this.removeWatch(pending.key);
        });
    }
    /**
    ...
    bindFlightPick() {
        document.addEventListener('click', (event) => {
            ...
            if (target.closest('button, a, input, .tail-chip, .map-switch'))
                return;
        ...
        document.addEventListener('change', (event) => {
            const box = event.target?.closest('input.map-show');
            if (!box)
                return;
        ...
        if (all)
            all.addEventListener('click', () => {
                ...
                if (this.mapHide.size > 0)
                    this.clearMapHide();
                else
                    this.clearFlightPick();
            });
    }
    ...
    emptyMessage(counts) {
        ...
        if (seen.mode !== 'all' && seen.mode !== 'noData' && counts.rows.length === 0 && counts.droppedBySeen > 0) {
            return (`Nothing on record has been seen within ${seen.phrase ?? seen.label}, so this window shows no ` +
    ...
    renderTypeList() {
        ...
        const measuredHtml = rows
            ...
            return (`<div class="typerow${already ? ' typerow-on' : ''}">` +
                ...
                bellButton(row.code, alerting && alertRule.tails.length === 0, alerting && alertRule.tails.length > 0) +
                '</div>' +
        ...
        // want an alert for"*.
        for (const button of host.querySelectorAll('.alert-toggle')) {
            button.addEventListener('click', () => {
                const code = button.dataset.type ?? '';
     ...
     */
    rowVisible(one) {
        if (one.type && this.mapHide.has(Page.showKey('type', one.type)))
            return false;
     ...
     */
    mapSwitch(kind, value, stateKind) {
     ...
     */
    toggleMapShow(key, on) {
        if (key === '')
            return;
        if (on)
            this.mapHide.delete(key);
        else
            this.mapHide.add(key);
    ...
    renderWatchlist() {
        ...
        const typeItems = this.typeRules
            ...
            const airborne = live.filter((one) => one.phase === 'airborne' && normaliseKey(one.type ?? '') === typeKey);
            const flyingRegs = new Set(airborne.map((one) => normaliseKey(one.registration)).filter((key) => key !== ''));
            const unidentified = airborne.filter((one) => normaliseKey(one.registration) === '').length;
            const quietWhy = `${unidentified} aircraft of this type ${unidentified === 1 ? 'is' : 'are'} airborne inside ` +
            ...
            for (const one of airborne) {
                const reg = (one.registration ?? '').trim();
                if (reg === '' || seen.has(normaliseKey(reg)))
                    continue;
                tails.push(reg);
    ...
    renderMap() {
        ...
        const hiddenNames = [...this.mapHide].map((key) => this.mapRowNames.get(key) ?? key.replace(/^(type|tail):/, ''));
        ...
        if (rowFilter) {
            ...
            for (const one of focused) {
                if (typeof one.lat !== 'number' || typeof one.lon !== 'number')
                    continue;
                points.push({ lat: one.lat, lon: one.lon });
                for (const point of one.trail ?? [])
        ...
        const placed = focused.filter((one) => typeof one.lat === 'number' && typeof one.lon === 'number');
        ...
        const rateByHex = new Map();
        for (const reading of this.lastReadings) {
            if (typeof reading.baro_rate === 'number')
                rateByHex.set(reading.hex.toLowerCase(), reading.baro_rate);
        }
        for (const one of placed.slice(0, 60)) {
            ...
            if (drawnType !== '')
                this.mapLastDrawn.set(drawnType, Date.now());
            ...
            const press = ` data-hex="${who}" title="${escapeHtml(picked ? 'Press to go back to all flights' : 'Press to put the map on this aircraft')}"`;
            planes +=
     ...
     */
    bindPlaceSearch() {
        const form = byId('placeSearchForm');
        const input = byId('placeSearchInput');
        ...
        form.addEventListener('submit', async (event) => {
            ...
            if (value.length < 2) {
                if (note)
                    note.textContent = 'Type at least two characters of a place name.';
                return;
            }
            clearResults();
            if (note)
                note.textContent = `Looking for ${value}…`;
            try {
                ...
                results.innerHTML = places
                    .map((place, index) => `<button type="button" class="chip chip-small place-result" ` +
                    `data-index="${index}" data-ga="place-result">` +
                    `${escapeHtml(place.label ?? place.name ?? '')}</button>`)
                    .join('');
                for (const button of results.querySelectorAll('.place-result')) {
                    button.addEventListener('click', () => {
     ...
     */
    async nameMyPosition(lat, lon, note) {
