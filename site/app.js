...
class Page {
    ...
    async poll() {
        if (!this.airport || !this.engine)
            return;
        const url = `/api/v2/point/${this.airport.lat}/${this.airport.lon}/${kmToNm(this.radiusKm)}`;
        try {
            ...
            if (response.status === 429) {
                ...
                this.setStatus('The feed asked us to slow down (HTTP 429). It is volunteer-funded and answers a limited number of requests; the page will try again on the next poll.', 'error');
                return;
            }
            if (!response.ok)
